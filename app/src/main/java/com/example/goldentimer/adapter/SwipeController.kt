...
//            p.color = Color.parseColor("#0074CD") //Paint 객체 컬러 지정, setColor 메서드는 int형 인자를 매개변수로 받기 때문에 string 형태의 색상을 Color 클래스의 parseColor 메서드를 이용해 int형으로 바꿔줌
            p.color = Color.BLUE
            c.drawRoundRect(leftButton, corners, corners, p) //drawRoundRect : 원 그리기
            drawText("공유", c, leftButton, p)
...
//            p.color = Color.parseColor("#FF473D")
            p.color = Color.RED
    ...
    private fun setTouchUpListener(
        ...
        recyclerView.setOnTouchListener { v, event ->
            ...
            if (buttonInstance != null && buttonInstance!!.contains(
                ...
                if (buttonsShowedState === ButtonState.LEFT_VISIBLE) {  //왼쪽버튼 클릭시(공유)
                    listener.onLeftClick(viewHolder.adapterPosition, viewHolder)
                } else if (buttonsShowedState === ButtonState.RIGHT_VISIBLE) {  //오른쪽버튼 클릭시(삭제)
